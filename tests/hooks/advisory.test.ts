...
import type { GateActionSignals } from "../../src/tools/gate-action-core.js";
...
describe("bands", () => {
  it("reads requested, requestedish, wide and out-of-scope the way the table does", () => {
    expect(bands(signals(NO, YES, 0.7, NO), 2.5, THRESHOLDS)).toEqual({
      requested: true,
      requestedish: true,
      wide: true,
      outOfScope: false,
    ...
    expect(bands(signals(0.6, YES, 0.7, NO), 1, THRESHOLDS)).toMatchObject({
      requested: false,
      requestedish: true,
    });
    expect(bands(signals(NO, NO, NO, NO), 1, THRESHOLDS)).toMatchObject({ outOfScope: true, requestedish: false });
  });
});
